...
our $config_blogTitle = 'Puppy Personal Blog';								# Blog title
our $config_adminPass = 'woofwoof';								# Admin password for adding entries
our $config_smiliesFolder = '/root/Web-Server/blog/smilies';								# Path to smilies, if you dont want smilies, just, dont upload that folder
our $config_smiliesFolderName = 'smilies';												# Smilies Folder Name, you should NOT change this...
our $config_postsDatabaseFolder = '/root/Web-Server/blog/posts';							# Name of the folder where entries will be saved. If you cant create entries with this path, try /home/www/user/posts
our $config_commentsDatabaseFolder = '/root/Web-Server/blog/comments';						# Name of the folder where entries will be saved. If you cant create entries with this path, try /home/www/user/posts
our $config_dbFilesExtension = 'ppl';								# Extension of the files used as databases
our $config_currentStyleFolder = '.';								# Styles folder (. = in the same path as ths file)
...
our $config_metaDescription = 'Puppy Linux Personal Blog';			# Also for search engines
...
our @config_commentsForbiddenAuthors = qw/admin administrator/;		# These are the usernames that normal users cant use, if you use one of these, it will ask for password
...
our @config_menuLinks = ('http://puppylinux.com/,Puppy Home', 'http://pplog.infogami.com/,Get PPLOG');										# Links to be displayed at the menu
...
our $config_showHits = 1;											# Want to show how many users are browsing your blog?			(0 = No, 1 = Yes)
our $config_sendMailWithNewComment = 0;								# Receive a mail when someone posts a comment					(0 = No, 1 = Yes) It works only if you host allows sendmail
our $config_sendMailWithNewCommentMail = 'bkauler@gmail.com';		# Email adress to send mail if allowed
...
our $config_gmt = +8;												# Your GMT, -3 = Buenos Aires
...
our $config_securityQuestionOnComments = 1;							# Allow the option to display a question which users have to answer in order to post comments
our $config_commentsSecurityQuestion = 'Name of the Puppy Linux mascot?';	# You shall change it, choose a question all your users will know
our $config_commentsSecurityAnswer = 'Puppy';						# Answer of the security question. The comparison will be CaSe InSeNsItIvE
...
our $config_useHtmlOnEntries = 0;									# Allow HTML on entries when making a new post (THIS WILL DISALLOW BBCODE!!)
our $config_useWYSIWYG = 0;											# You must allow HTML on entries for this to work // Note, WYSIWYG wont allow smilies
...
print '</div><div id="footer">Copyright '.$config_blogTitle.' 2008 - All Rights Reserved - Powered by <a href="http://pplog.infogami.com/">PPLOG</a>'; print '<br>All posts are using GMT '.$config_gmt if $config_showGmtOnFooter == 1; print '</div></div></body></html>';
}